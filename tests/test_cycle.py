...
def observation(
    ...
    *,
    running: bool,
    kind: MachineSignalKind = MachineSignalKind.CONTROLLER,
...
def test_cycle_start_and_finish_are_debounced() -> None:
    ...
    assert started.event.event_type is EventType.MACHINE_CYCLE_STARTED
    ...
    assert finished.event.event_type is EventType.MACHINE_CYCLE_FINISHED


def test_visual_machine_state_is_explicitly_inferred() -> None:
    trigger = DebouncedCycleTrigger(
        "cnc-03",
        config=CycleTriggerConfig(start_stability_seconds=0.0),
    )
    result = trigger.process(
        observation(0.0, running=True, kind=MachineSignalKind.VISUAL),
    )
...
def test_low_confidence_observation_does_not_advance_state() -> None:
    trigger = DebouncedCycleTrigger(
        "cnc-03",
        config=CycleTriggerConfig(start_stability_seconds=0.0, min_confidence=0.8),
    )
...
def test_simulated_signal_is_a_safe_development_double() -> None:
    signal = SimulatedMachineSignal("dev-cnc")
    result = signal.observe(True, timestamp=BASE)

    assert result.running is True
    assert result.kind is MachineSignalKind.SIMULATED
